...
weights -- i.e. weight that have been designed to enable computing
...
errors will not reflect the correct values. Users wanting to use SPSS
...
that at the time of writing this document only some of the BSA editions
...
`mrdoc/ukda_data_dictionaries.zip` You may need to decompress the file
separately
...
about the initial i.e. primary sampling units.\
This is achieved with the `CSPLAN` command through we create a plan file
which contains the survey design information.
...
will compute the average (i.e. mean) age of respondents in the sample,
as well as the proportion of male and female respondents aged over 55.
We will need to use `/CSDESCRIPTIVES`
...
Under the `/STATISTICS` command we can request either or both the
...
this case, but we know that a 95% confidence interval is the sample mean
respectively minus and plus 1.96 times its standard error. Using the
SPSS output, we can compute it ourselves as 1.96\*.2872=about .56 years,
that is close to 7 months.
...
**Question 3** What would be the consequences of weighting but not
...
is the `Politics` variable. It has five categories that we are going to
recode into 'Significantly' (interested) and 'Not' (significantly) in
...
With the help of `CSTABULATE` we can examine frequencies, proportions
and confidence intervals of these proportions accounting for the survey
design. As before, the point estimates do not further change once survey
...
##  Domain (i.e. subpopulation) estimates
...
The SPSS command `CSDESCRIPTIVES` that we used above makes such domain
estimation relatively straightforward. If we would like to compute the
mean age of BSA respondents by government office regions, we need to
specify:
...
smallest?
  ...
  When using `CSTABULATE`, we can define domains or subpopulations with
several variables, not just one. For example, we could look at gender
differences in political affiliations by regions. However, as the size
of subgroups decrease, so does the precision of the estimates as their
confidence interval widens, to a point where their substantive interest
